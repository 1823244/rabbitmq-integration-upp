...
Функция получитьRoutingKeyПоТипуОбъекта(ТипОбъектаМД)
	...
	Если СтрНайти(НРег(ТипОбъектаМД), "справочник") > 0  Тогда
		Возврат  "static";
	ИначеЕсли СтрНайти(НРег(ТипОбъектаМД), "документ") > 0  Тогда
		Возврат  "doc";
	ИначеЕсли СтрНайти(НРег(ТипОбъектаМД), "регистрнакопления") > 0 Тогда
		Возврат  "doc";
	КонецЕсли;
...
Функция СоздатьУзелIdentification_Документа(СсылкаНаОбъект)	
...
Функция СоздатьУзелDefinition_Документа(СсылкаНаОбъект)
	...
	Если ТипЗнч(СсылкаНаОбъект.Номер) = Тип("Число") Тогда
		ОтветСтруктура.Вставить("Number", формат(СсылкаНаОбъект.Номер,"ЧГ=;ЧРГ=;ЧН="));
	Иначе 
...
//ЕНС ++ Подготовка файла json с остатками по регистру нак. ТоварыПереданные
Процедура ВыгрузитьОстаткиТоваровПереданных(ДатаОстатков = Неопределено, Контрагент = Неопределено, Организация = Неопределено) Экспорт
	...
	|	рег.СуммаВзаиморасчетовОстаток КАК СуммаВзаиморасчетов
	|ИЗ
	|	РегистрНакопления.ТоварыПереданные.Остатки(&ДатаОстатков, &УсловиеКонтрагент и &УсловиеОрганизация) КАК рег";

	Если ЗначениеЗаполнено(Контрагент) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст , "&УсловиеКонтрагент", "Контрагент = &Контрагент");
	Иначе 
		Запрос.Текст = СтрЗаменить(Запрос.Текст , "&УсловиеКонтрагент", "1=1");
	КонецЕсли;
	Запрос.УстановитьПараметр("Контрагент", Контрагент);

	Если ЗначениеЗаполнено(Организация) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст , "&УсловиеОрганизация", "Организация = &Организация");
	Иначе 
		Запрос.Текст = СтрЗаменить(Запрос.Текст , "&УсловиеОрганизация", "1=1");
	КонецЕсли;
	...
	Попытка
		...
		СтруктураОбъекта = Новый Структура;
		СтруктураОбъекта.Вставить("source", "UPP");
		СтруктураОбъекта.Вставить("type", "РегистрНакопления.ТоварыПереданные");
		СтруктураОбъекта.Вставить("logmoment", XMLстрока(ТекущаяДата()));
		
		identification = Новый Структура("type,Ref", "", "");
		identification.type = "РегистрНакопления.ТоварыПереданные";
		СтруктураОбъекта.Вставить("identification", identification);
		...
		definition.Вставить(ТЧТовары);
		
		Пока Выборка.Следующий() Цикл
			
			СтрокаТЧ = Новый Структура;
			
			СтрокаТЧ.Вставить("Номенклатура", СоздатьУзелDefinition(Выборка.Номенклатура));
			СтрокаТЧ.Вставить("ХарактеристикаНоменклатуры", СоздатьУзелDefinition(Выборка.ХарактеристикаНоменклатуры));
			СтрокаТЧ.Вставить("Количество", Выборка.Количество);
			СтрокаТЧ.Вставить("СуммаВзаиморасчетов", Выборка.СуммаВзаиморасчетов);
			
			ТЧТовары.Добавить(СтрокаТЧ);
		...
		SendToRabbitOneObject(json, "РегистрНакопления.ТоварыПереданные");
